...
def run() :
	...
	V = st.sidebar.selectbox('Level of ventilation:',
				( 'None','Low',
                    		'Normal', 'High'),index=2)
	ID_0 = st.sidebar.slider('Indoor Tempereture(C):', 15, 35, 25)
	UD_0 = st.sidebar.slider('Outdoor Tempereture(C):', -10, 35, 10)
...
ID_1 = st.sidebar.slider('Please specify the Indoor Tempereture(C):', 15, 35, 10)	
UD_1 = st.sidebar.slider('Please specify the Outdoor Tempereture(C):', -10, 35, 0)
with st.spinner('Wait for it...'):
